...
中间件还可心接收额外的参数。例如，如果你的应用程序在执行给定的动作之前需要验证认证的用户是否具有一个给定的『角色』，你可以创建一个 `CheckRole` 中间件，它接受一个角色名称作为一个额外参数。
...
`terminate` 方法应该同时接收请求和响应。一旦你定义了一个可终止中间件，你应该添加它到路由列表或者在 `app/Http/Kernel.php` 文件全局中间件中。
