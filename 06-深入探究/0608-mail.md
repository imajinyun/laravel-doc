...
如果你希望始终排队你的可邮件类，可以在该类上实现 `ShouldQueue` 契约。现在，即使你在邮寄时调用 `send` 方法，可邮件仍然会被排队，因为它实现了契约：
...
有时，应用程序存储每个用户的首选区域设置。通过在一个或多个模型上实现 `HasLocalePreference` 契约，你可以指示 Laravel 在发送邮件时使用这个存储的区域设置：
