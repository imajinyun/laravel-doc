...
注意我们将容器本身作为一个解析器的参数接收。然后我们可以使用容器来解析我们正在构建的对象的子依赖项。
...
有时你可能有一个接收一些注入类的类，但也需要一个注入的原始值如整数。你可以轻松使用上下文绑定来注入类可能需要的任何值：
...
偶尔，你可能需要去解决所有绑定的一个确定『分类』。例如，也许你正在构建一个报告聚合器，该聚合器接收许多不同 `Report` 接口实现的数组。注册 `Report` 实现后，你可以使用 `tag` 方法为它们分配一个标记：
